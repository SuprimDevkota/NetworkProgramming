...
void processRequest(asio::ip::tcp::socket& sock)
{
	...
	//This means that the peer application has shut down its socket
	if (ec != asio::error::eof) {
		throw system::system_error(ec);
	}

	// Request received. Sending response. Allocating and filling the buffer with
	// binary data.
	...
	//Shutting down the socket to let the client know that we have sent the response
